...
const QUOTES = [
  ...
  // --------------------------------------------------------- Calendar joke ---
  [CAL_JOKE, '"Gym" has recurred since March. The RRULE is the only consistent one here.'],
  [CAL_JOKE, 'All-day event. Like your to-do list. Forever.'],
  [CAL_JOKE, "This note is TRANSP:TRANSPARENT, so it won't block your day. Unlike your inbox."],
  [CAL_JOKE, "Reminder set for -10 minutes: you're already late in principle."],
  [CAL_JOKE, "Your busiest day is marked 'tentative.' Emotionally accurate."],
  [CAL_JOKE, 'Someone invited you with PARTSTAT=NEEDS-ACTION. Same, honestly.'],
  [CAL_JOKE, 'This event repeats UNTIL you deal with it. No COUNT. No mercy.'],
  ...
  [CAL_JOKE, 'DTSTART: now. DTEND: whenever you stop scrolling.'],
  [CAL_JOKE, "Your 9am is recurring. So is your 8:55 panic. Beautiful symmetry."],
  [CAL_JOKE, "That 'quick sync' has no DTEND. A meeting with COUNT=infinity."],
  [CAL_JOKE, "You declined the event but kept the dread. Very RFC-noncompliant."],
  [CAL_JOKE, "BYDAY=MO,TU,WE,TH,FR. The weekend is an EXDATE, cherish it."],
  [CAL_JOKE, "You set three reminders. You will ignore all three, in order."],
  [CAL_JOKE, "This block says 'focus time.' The RRULE says otherwise."],
  [CAL_JOKE, "Every calendar has one event titled just '???'. This is a safe space."],
  [CAL_JOKE, "Leap day is here: recurring-birthday edge case in the flesh."],
  [CAL_JOKE, "DST tonight. One hour of sleep is being force-pushed. No revert."],
  ...
  [CAL_JOKE, "This note has no ATTENDEEs. Just you, being quietly cheered on."],
  [CAL_JOKE, "You moved the workout to 'later.' Later has UNTIL=never."],
  [CAL_JOKE, "Free/busy status: busy being free of any actual plans."],
  [CAL_JOKE, "Someone scheduled a meeting to plan the meeting. Recursion detected."],
  [CAL_JOKE, "Your 'out of office' is the most productive thing on the calendar."],
  [CAL_JOKE, "Overlapping events again. The universe supports concurrency; you don't."],
  [CAL_JOKE, "RECURRENCE-ID: the one Tuesday that thinks it's special. It is."],
  ...
  for (let y = startYear; y < startYear + years; y++) {
    ...
    for (const {date, kind, text} of events) {
      ...
      lines.push(
        ...
        `SUMMARY:${esc(text)}`,
        `CATEGORIES:${esc(CAL_NAME)},${esc(kind)}`,
        'TRANSP:TRANSPARENT',
        'END:VEVENT',
      );
      count++;
