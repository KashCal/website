---
slug: natural-language-calendar-events-android
title: "Say it once. Skip the form. It's fantastic."
authors: [kashcal]
tags: [features, releases]
keywords:
  - natural language calendar android
  - add calendar event by typing
  - type events in plain english
  - quick add calendar app android
...
- **Location** after the word `at`: `Lunch at Olive Garden` files the place, not the title.
- **Tags** with `#`: `Deep work #focus` keeps the tag and drops the rest into the event.

Nothing to tap through. The sentence is the form.

## The repeats that used to need a dialog

Recurring events are where "just fill out the form" gets especially tedious, so this is where typing pays off most. KashCal now understands the awkward ones:

- `every weekday`, `every 2 weeks`, `monthly until December`
- `2nd Tuesday of the month` for the meeting that never lands on the same date
- `last Thursday of the month`, the one your old calendar could not express without a calendar of its own
- `last day of the month` for rent, whether that month ends on the 30th, the 31st, or a nervous 28th

Say it plainly and KashCal sets the repeat. No radio buttons were harmed.

## A place for the part that isn't scheduling

Sometimes the event carries a thought that is not a time or a place. Add it after a space and `//`, and everything past it becomes the note:

`Call plumber tomorrow 3pm // ask about the leak under the sink`

KashCal only reads the part before the `//` for the schedule, so words in your note never quietly move the event. Paste a link like `https://...` and it stays put, because the space before `//` is what marks the note.

## It looks and works, well, fantastic
